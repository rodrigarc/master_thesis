...
knitr::opts_knit$set(root.dir = "C:/Users/rodri/Desktop/sanger_LC")
...
sf <- summarise.abi.folder("/Users/rodri/Desktop/sanger_LC/animals_LC",
...
#create a column based on the the plate name
   mutate(plate = substring(folder.name,5,6))%>%
  mutate(well = ifelse(nchar(file.name) < 44,
                       substring(file.name, 1, 2),
                       substring(file.name, 1, 3))) %>%
  mutate(ID = substring(folder.name, 1, 3)) %>%
  mutate(chain = str_sub(folder.name, -2, -1)) %>%
  mutate(file_ID = sub("_KC","",folder.name)) %>%
  mutate(file_ID = sub("_LC","",file_ID)) %>%
  mutate(file_ID = paste(file_ID, well, chain, sep = "_"))
...
sf$summaries %>% ggplot(aes(raw.length))+geom_bar()
```
...
mapply(FUN = secondary.peaks, 
       ...
       MoreArgs = list(output.folder = "/Users/rodri/Desktop/sanger_LC/quality_reports/secondary_peaks", ratio = 0.5))
...
rmarkdown::render("/Users/rodri/Desktop/sanger_LC/code/sanger_LC.rmd",
                  output_file = paste("LC","_","report_", '_', Sys.Date(), ".html", sep = ""), 
                  output_dir = "/Users/rodri/Desktop/sanger_LC/quality_reports/quality.rmd")
getwd()
...
sf_filtered %>% select(file_ID,ID, plate, well, chain, raw.mean.quality, trimmed.mean.quality,sequence) %>%
write.csv("/Users/rodri/Desktop/sanger_LC/quality_reports/sanger_filtered.csv")
```

```{r}
lc <- read.csv("/Users/rodri/Desktop/sanger_LC/quality_reports/sanger_filtered.csv")
lc <- lc %>% select(file_ID, sequence)
lc$file_ID <- as.character(lc$file_ID)
lc$sequence <- as.character(lc$sequence)

dataframe2fas(lc, file = "/Users/rodri/Desktop/sanger_LC/quality_reports/E16_LC.fasta")
