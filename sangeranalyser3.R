...
summarise.abi.folder <- function(input.folder, trim.cutoff = 0.0001, secondary.peak.ratio = 0.33, write.secondary.peak.files = FALSE, processors = NULL){
  ...
  print("Loading reads...")
  abi.seqs = lapply(abi.fnames, read.abif)
  ...
  qual_scores = lapply(summaries.dat, function(x) x[["quality_score"]])
  names(qual_scores) = as.character(abi.fnames)
  return(list("summaries" = summaries, "reads" = reads, "quality_scores" = qual_scores))
...
summarise.abi.file <- function(seq.abif, trim.cutoff = 0.0001, secondary.peak.ratio = 0.33, output.folder = NA, prefix = "seq", processors = NULL){
  ...
  qual_position = c("score" = qual, 
                    "position" = c(1:length(seq.sanger@primarySeq)))
  return(list("summary" = read.summary, "read" = seq.sanger, "quality_score" = qual_x ))
