...
sf <- summarise.abi.folder("~/Desktop/rodrigo/sanger_HC/sanger_HC",                           secondary.peak.ratio = 0.33, 
                           trim.cutoff = 0.01)
...
```{r plot_ID_quality, echo=FALSE, warning=FALSE, fig.align='center'}
...
g1 <- sf$summaries %>%
  ggplot(aes(x = ID, y = trimmed.mean.quality)) + 
  geom_boxplot() +
  ylim(0,60)+
...
```{r plate_quality, echo=FALSE, warning=FALSE, fig.align='center'}
#This code is to create a general table containing the quality report per plate

#Calculate the mean quality per ID number
ID_scores <- sf_filtered %>% 
  group_by(ID) %>%
...
#Calculate the number of repeated sequences per ID
...
#calculate the total number of sequences per ID
total.seq <- sf$summaries %>%
  group_by(ID) %>%
  summarise(n_total = n()) %>%
  data.frame()

#calculate the number of sequences post filtering per ID
...
x <- merge(n.repeated.seq, total.seq , by.default = ID, all = TRUE)
x <- merge(x, filtered.seq, by.default = ID, all = TRUE)
x[is.na(x)] <- 0
...
table.seq <- merge(plates_used_sequences, ID_scores, by.default = ID)
kable(table.seq) %>%
...
mapply(FUN = secondary.peaks.diff, 
       ...
       MoreArgs = list(output.folder = "/Users/rodri/Desktop/sanger_HC/quality_reports/HC_200506_batch/"))
...
sf_summaries_csv <- sf$summaries %>%
  select(file.path, file_ID,ID, plate, well, chain, raw.mean.quality, trimmed.mean.quality) 

combined_sorts <- read.csv("/Users/rodri/Desktop/sanger_HC/specificity/combined_sorts_4.csv")

diff <- anti_join(sf_filtered_csv,merged_csv,  by = "file_ID")
View(diff)

merged_csv <- merge(sf_filtered_csv, combined_sorts, by.default="file_ID") %>% 
  select(file.path,file_ID,ID, plate, well, chain, specificity, raw.mean.quality, trimmed.mean.quality,sec.peak.CDR3, sequence)

merged_csv <- merged_csv %>% mutate(full_ID = paste(file_ID,specificity, sep = "_"))
merged_csv %>% select(full_ID,ID, plate, well, chain, specificity, raw.mean.quality, trimmed.mean.quality,sec.peak.CDR3, sequence) %>% write.csv("/Users/rodri/Desktop/sanger_HC/quality_reports/HC_200506_batch/sanger_filtered.csv")

```
Obs.: After merging, sequences without a index sorting (specificity) were deleted. Those sequences were in general empty wells but also double negative. In total `r nrow(sf_filtered)-nrow(merged_csv)` sequences were subtracted from the total filtered sequences, thus the csv file contains `r nrow(merged_csv)`.
...
#fasta file creation
filtered.filepath <- as.character(merged_csv$file.path)
sangerseqfiltered <- sapply(filtered.filepath, readsangerseq)
sangerbasecallfasta <- sapply(sangerseqfiltered, primarySeq)
name_fasta <-  merged_csv %>% mutate(name = paste(file_ID,sep = "_",specificity)) 
names(sangerbasecallfasta) <- name_fasta$name
  
write.dna(sangerbasecallfasta, 
          file = "/Users/rodri/Desktop/sanger_HC/fasta/HC_200506_batch/HC.fasta", 
          format = 'fasta', 
          nbcol = -1, 
          colsep = "",
          colw = 10000000,
...
```
## Quality Reports per animal


```{r loop_report, echo=FALSE, include=FALSE, eval = FALSE}
rmarkdown::render("/Users/rodri/Desktop/sanger_HC/code/template_all.Rmd",output_file = paste("Complete","_","report_", '_', Sys.Date(), ".html", sep = ""), output_dir = '/Users/rodri/Desktop/sanger_HC/quality_reports/HC_200506_batch/')
