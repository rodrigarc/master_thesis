...
* Trim start less than 65 and trim finish more than 150
* trimmed secondary peaks less than 150
...
sf <- summarise.abi.folder("~/Box Sync/Rodrigo & Klara & Sebastian/Raw data/Sanger sequencing/Public",
...
sf$summaries <- sf$summaries %>%
  mutate(plate = substr(file.name, 5,6))%>%
  mutate(well = substr(file.name, 8,9)) %>%
  mutate(chain = substr(folder.name, 8,9)) %>% 
  mutate(ID = substr(file.name, 1,3))%>%
  mutate(file_ID = substr(file.name, 1, 12))
...
mapply(FUN = secondary.peaks, 
       ...
       MoreArgs = list(output.folder = "~/Box Sync/Rodrigo & Klara & Sebastian/Quality reports/Light_chains/public_clones/chromatograms/", ratio = 0.5))
...
rmarkdown::render("~/Box Sync/codes/2020-08-28_template_LC_public.Rmd",
                  output_file = paste("LC","_","report_", '_', Sys.Date(), ".html", sep = ""), 
                  output_dir = "~/Box Sync/Rodrigo & Klara & Sebastian/Quality reports/Light_chains/public_clones/")

```


```{r , echo=FALSE, warning=FALSE, include=FALSE, eval=FALSE}
sf_filtered %>% mutate(sample = file_ID) %>%
  select(file_ID, ID, plate, well, chain, raw.mean.quality, trimmed.mean.quality,sequence) %>%
write.csv("~/Box Sync/Rodrigo & Klara & Sebastian/Quality reports/Light_chains/public_clones/sanger_filtered.csv", row.names = F)
...
lc <- read.csv("~/Box Sync/Rodrigo & Klara & Sebastian/Quality reports/Light_chains/public_clones/sanger_filtered.csv")
lc <- lc %>% select(file_ID, sequence)
lc$file_ID <- as.character(lc$file_ID)
lc$sequence <- as.character(lc$sequence)

dataframe2fas(lc, file = "~/Box Sync/Rodrigo & Klara & Sebastian/Quality reports/Light_chains/public_clones/pub_LC.fasta")
