...
sf <- summarise.abi.folder("~/Box Sync/RSV NGS/CureVac COVID19/ab1 files/LC/",
...
sf$summaries <- sf$summaries %>%
  mutate(plate = substr(folder.name, 1,7))%>%
  mutate(well = gsub("\\-.*|\\_.*", "", file.name)) %>%
  mutate(chain = substr(folder.name, 9,10)) %>% 
...
mapply(FUN = secondary.peaks, 
       ...
       MoreArgs = list(output.folder = "~/Box Sync/RSV NGS/CureVac COVID19/processing/LC/chromatograms/", ratio = 0.5))
...
rmarkdown::render("~/Box Sync/codes/2020-08-18_template_LC_COVID.Rmd",
                  ...
                  output_dir = "~/Box Sync/RSV NGS/CureVac COVID19/processing/LC/")
...
sf_filtered %>% mutate(sample = file_ID) %>%
  select(file_ID, plate, well, chain, raw.mean.quality, trimmed.mean.quality,sequence) %>%
write.csv("~/Box Sync/RSV NGS/CureVac COVID19/processing/LC/sanger_filtered.csv", row.names = F)
